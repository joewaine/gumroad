...
describe AssetPreview, :vcr do
  describe "Attachment" do
    ...
    it "fails with an arbitrary filetype" do
      ...
      expect(asset_preview.errors.full_messages).to eq(["Covers must be an image (JPEG, PNG, GIF) or video. The file you provided is not a supported format."])
    end
    ...
    describe "#analyze_file" do
      ...
      it "fails with a script disguised as an image" do
        ...
        asset_preview.file.attach(blob)
        expect(asset_preview.save).to eq(false)
        expect(asset_preview.errors.full_messages).to include("Covers must be an image (JPEG, PNG, GIF) or video. The file you provided is not a supported format.")
      end
